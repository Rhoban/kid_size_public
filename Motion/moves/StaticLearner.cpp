...
StaticLearner::StaticLearner()
{
  ...
  bind->bindNew("s", _staticOn, RhIO::Bind::PullOnly)
      ->comment("If on, the robot will stop moving. If off, the robot will be movable but still hard.")
      ->persisted(true)
      ->defaultValue(1)
      ->minimum(0)
      ->maximum(1);
  ...
  bind->bindNew("a", _staticOnArms, RhIO::Bind::PullOnly)
      ->comment("Like the s command but only for arms")
      ->persisted(true)
      ->defaultValue(1)
      ->minimum(0)
      ->maximum(1);
  ...
  bind->bindNew("l", _staticOnLeftLeg, RhIO::Bind::PullOnly)
      ->comment("Like the s command but only for the left leg")
      ->persisted(true)
      ->defaultValue(1)
      ->minimum(0)
      ->maximum(1);
  ...
  bind->bindNew("r", _staticOnRightLeg, RhIO::Bind::PullOnly)
      ->comment("Like the s command but only for the right leg")
      ->persisted(true)
      ->defaultValue(1)
      ->minimum(0)
      ->maximum(1);
  ...
  bind->bindNew("i", _staticPointIndex, RhIO::Bind::PullOnly)
      ->comment("Increment this index to take a snapshot")
      ->persisted(true)
      ->defaultValue(0)
      ->minimum(0)
      ->maximum(100);
  ...
  bind->bindNew("snapshot", _snapshot)
      ->comment("Sete to true to take a snapshot at the current index")
      ->persisted(true)
      ->defaultValue(0)
      ->minimum(0)
      ->maximum(1);
  ...
  bind->bindNew("goToOn", _goToOn, RhIO::Bind::PullOnly)
      ->comment("If on, the robot will go to the recorded position of index 'p'")
      ->persisted(true)
      ->defaultValue(0)
      ->minimum(0)
      ->maximum(1);
  ...
  bind->bindNew("p", _goToIndex)
      ->comment("Index of the position the robot shall go to is goToOn is true")
      ->persisted(true)
      ->defaultValue(0)
      ->minimum(0)
      ->maximum(10000);
...
void StaticLearner::onStart()
{
  ...
  _dofNames = { "left_hip_pitch",      "left_hip_roll",      "left_knee",         "left_ankle_pitch",
                "left_ankle_roll",     "left_hip_yaw",       "right_hip_yaw",     "right_hip_pitch",
                "right_hip_roll",      "right_knee",         "right_ankle_pitch", "right_ankle_roll",
                "left_shoulder_pitch", "left_shoulder_roll", "left_elbow",        "right_shoulder_pitch",
                "right_shoulder_roll", "right_elbow" };

  _legsDofNames = {
    "left_hip_pitch", "left_hip_roll",   "left_knee",      "left_ankle_pitch", "left_ankle_roll",   "left_hip_yaw",
    "right_hip_yaw",  "right_hip_pitch", "right_hip_roll", "right_knee",       "right_ankle_pitch", "right_ankle_roll",
  };

  _leftLegDofNames = { "left_hip_pitch",   "left_hip_roll",   "left_knee",
                       "left_ankle_pitch", "left_ankle_roll", "left_hip_yaw" };

  _rightLegDofNames = { "right_hip_yaw", "right_hip_pitch",   "right_hip_roll",
                        "right_knee",    "right_ankle_pitch", "right_ankle_roll" };

  _armsDofNames = { "left_shoulder_pitch",  "left_shoulder_roll",  "left_elbow",
  ...
  for (auto name : _dofNames)
  {
    ...
    std::cout << name
              << " (torque) = " << getScheduler()->getManager()->dev<MOTOR>(name).torqueLimit().readValue().value
...
void StaticLearner::step(float elapsed)
{
  ...
  else
  {
    ...
    {
      // /!\ since a, l and r were introduced, addExtraAngles should be rewritten accordingly to avoid the double
      // setAngle of death. It didn't work well so not worth it.
...
void StaticLearner::calculateExtraAngles()
{
  ...
  for (auto name : _dofNames)
  {
     ...
     */
    /*if (name == "left_elbow") {
      std::cout << "Goalpos = " << goalPos << ", delta = " << delta << ", extraAngle = " << _mapOfExtraAngles[name] <<
      std::endl;
      }*/
  }
}
