...
KickCalibration::KickCalibration(Approach* approach) : approach(approach), kickController(NULL)
{
  initializeBinding();

  bind->bindNew("startX", startX, RhIO::Bind::PullOnly)
      ->comment("X Offset for the starting")
      ->defaultValue(75)
      ->persisted(true);

  bind->bindNew("started", started, RhIO::Bind::PullOnly)->comment("Start approach")->defaultValue(false);
}
...
void KickCalibration::step(float elapsed)
{
  ...
  if (!logging)
  {
    ...
    if (currentState.compare("shoot") == 0 && lastState.compare("place") == 0)
    {  // Starting to shoot
    ...
    if (currentState.compare("place") == 0 && lastState.compare("shoot") == 0)
    {  // Just shot
