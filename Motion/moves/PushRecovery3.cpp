...
PushRecovery3::PushRecovery3(Walk* walk) : walk(walk)
{
  initializeBinding();

  bind->bindNew("pushWalkEnable", pushWalkEnable)->comment("pushWalkEnable")->defaultValue(false)->persisted(true);

  bind->bindNew("enableReaction", enableReaction)->comment("enableReaction")->defaultValue(false)->persisted(true);

  bind->bindNew("logTime", logTime, RhIO::Bind::PullOnly)->comment("logTime")->defaultValue(30)->persisted(true);

  bind->bindNew("offsetTrunkPitch", offsetTrunkPitch, RhIO::Bind::PullOnly)
      ->comment("offsetTrunkPitch")
      ->defaultValue(0)
      ->persisted(true);

  bind->bindNew("offsetTrunkZ", offsetTrunkZ, RhIO::Bind::PullOnly)
      ->comment("offsetTrunkZ")
      ->defaultValue(0.02)
      ->persisted(true);

  bind->bindNew("thresholdNominalScore", thresholdNominalScore, RhIO::Bind::PullOnly)
      ->comment("thresholdNominalScore")
      ->defaultValue(10)
      ->persisted(true);
  ...
  bind->bindNew("offsetHip", offsetHip, RhIO::Bind::PullOnly)->comment("offsetHip")->defaultValue(25)->persisted(true);
    ...
    if (enableLog)
    {
      ...
      {  // logging
        ...
        if (!waiting)
        {
          ...
          v << pitch, accX, pressureLeft, pressureRight;
    ...
    {  // Not logging
      ...
      {  // Json Loaded
        ...
        if (recoverFall)
        {
          ...
          if (walk->getPhase() > 0.15 && walk->getPhase() < 0.35)
          {  // TODO Tuner seuils
          ...
          else if (walk->getPhase() > 0.65 && walk->getPhase() < 0.85)
          {  // TODO Tuner seuils
        ...
        if (triggerLeftFoot)
        {
          if (enableReaction)
          {
            ...
            std::cout << "normalTrunkPitchBackwards+offsetTrunkPitch :" << normalTrunkPitchBackwards + offsetTrunkPitch
                      << std::endl;
            walk->setTrunkPitchBackward(normalTrunkPitchBackwards + offsetTrunkPitch);
          }
