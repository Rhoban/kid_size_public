...
Move::Move() : Helpers(), ElapseTick(), bind(nullptr), _isRunning(false), _error()
{
}
...
bool Move::tick(double elapsed)
{
  if (isRunning())
  {
    ...
    {
      _error = "Exception runtime_error: " + string(exc.what());
    }
    catch (const std::logic_error& exc)
    {
      _error = "Exception logic_error: " + string(exc.what());
    }
    catch (const std::string& exc)
    {
      _error = "Exception string: " + exc;
    }
    catch (...)
    {
...
void Move::initializeBinding()
{
  if (bind != nullptr)
  {
    throw std::logic_error("Move already initialize: " + this->getName());
  }

  bind = new RhIO::Bind("moves/" + this->getName());

  bind->bindFunc(this->getName(), "Starts the move " + this->getName(), &Move::cmdStart, *this);
}
