...
Approach::Approach(Walk* walk, Head* head) : ApproachMove(walk), head(head)
{
  ...
  // Align
  bind->bindNew("farAlignerP", farAlignerP, RhIO::Bind::PullOnly)->defaultValue(0.4)->persisted(true);
  bind->bindNew("rotateAlignerP", rotateAlignerP, RhIO::Bind::PullOnly)->defaultValue(0.4)->persisted(true);
  bind->bindNew("nearAlignerP", nearAlignerP, RhIO::Bind::PullOnly)->defaultValue(0.2)->persisted(true);
  bind->bindNew("rotateLateralP", rotateLateralP, RhIO::Bind::PullOnly)->defaultValue(-0.6)->persisted(true);
  bind->bindNew("nearLateralP", nearLateralP, RhIO::Bind::PullOnly)->defaultValue(0.6)->persisted(true);
  // Walk to the ball

  bind->bindNew("stepP", stepper.k_p, RhIO::Bind::PullOnly)->defaultValue(1)->persisted(true);
  // Radius
  bind->bindNew("radius", radius)->defaultValue(0.45)->minimum(0.0)->maximum(1.0);
  // Time interval between shoots
  bind->bindNew("shootInterval", shootInterval, RhIO::Bind::PullOnly)->defaultValue(3)->persisted(true);

  // Centering corner distance
  bind->bindNew("centeringCorner", centeringCorner, RhIO::Bind::PullOnly)->defaultValue(2.50)->persisted(true);
  ...
  bind->bindNew("idiot", idiot, RhIO::Bind::PullOnly)
      ->comment("Attack with only the magnetometer")
      ->defaultValue(false)
      ->persisted(true);
  // Penalty mode
  ...
  bind->bindNew("lateralKickTreshold", lateralKickTreshold, RhIO::Bind::PullOnly)
      ->comment("Choose the range where we will do lateral kick")
      ->defaultValue(75)
      ->persisted(true);

  bind->bindNew("enableLateralKick", enableLateralKick, RhIO::Bind::PullOnly)
      ->comment("Enable the lateral kicks")
      ->defaultValue(true)
      ->persisted(true);
  ...
  if (state == STATE_FAR)
  {
    ...
    else
    {
      if (enableLateralKick)
      {
        ...
        if (IA.getLength() < target.getLength())
          target = IA;
        if (IB.getLength() < target.getLength())
          target = IB;
      }
