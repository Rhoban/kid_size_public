...
LateralStep::LateralStep() : lateralStepModel(Leph::SigmabanModel), inv(lateralStepModel)
{
  initializeBinding();
  ...
  */
  ...
  bind->bindFunc("LateralStepGen", "Pre-compute the lateral step", &LateralStep::cmdLateralStepGen, *this);
...
void LateralStep::loadLateralStep(std::string filename)
{
  ...
  auto leftOrientation =
      MatrixToEuler(lateralStepModel.orientation("left_foot_tip", "origin").transpose(), EulerYawPitchRoll);
  auto rightOrientation =
      MatrixToEuler(lateralStepModel.orientation("right_foot_tip", "origin").transpose(), EulerYawPitchRoll);
  ...
  while (t < xMax)
  {
    ...
    if (lateralStepModel.getDOF("left_knee") < 0.01)
      lateralStepModel.setDOF("left_knee", 0.01);
    if (lateralStepModel.getDOF("right_knee") < 0.01)
      lateralStepModel.setDOF("right_knee", 0.01);
    inv.run(0.0001, 100);
