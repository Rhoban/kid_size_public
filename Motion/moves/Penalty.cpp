...
Penalty::Penalty(PenaltyKickController* controler) : controler(controler)
{
  initializeBinding();

  startX = 1.35;

  bind->bindNew("forceStart", forceStart, RhIO::Bind::PullOnly)->comment("Force the start")->defaultValue(false);
}
      ...
      if (watching)
      {
        if (T > 1)
        {
          ...
          if (opponents.size() == 1)
          {
            ...
            y = y / opponents.size();
            bool left = y < 0;
            logger.log("Saw %d opponents, kicking at %s", opponents.size(), left ? "left" : "right");
