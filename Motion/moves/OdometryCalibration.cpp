  ...
  , _randomWalk(3)
{
  ...
  bind->bindFunc("calibration_odometry_start", "Start logging a learning sequence", &OdometryCalibration::cmdStartLog,
                 *this);
  bind->bindFunc("calibration_odometry_stop", "Stop logging a learning sequence", &OdometryCalibration::cmdStopLog,
                 *this);
  bind->bindFunc("calibration_odometry_cancel", "Stop logging and pop last started learning sequence.",
                 &OdometryCalibration::cmdCancelLog, *this);
  bind->bindFunc("calibration_odometry_dump", "Dump recorded odometry data into given filename",
  ...
  bind->bindNew("autoInertia", _autoInertia, RhIO::Bind::PullOnly)->defaultValue(0.0)->persisted(true);
  bind->bindNew("autoStepMin", _autoMinStep, RhIO::Bind::PullOnly)->defaultValue(-0.02)->persisted(true);
  bind->bindNew("autoStepMax", _autoMaxStep, RhIO::Bind::PullOnly)->defaultValue(0.04)->persisted(true);
  bind->bindNew("autoLateralMin", _autoMinLateral, RhIO::Bind::PullOnly)->defaultValue(-0.02)->persisted(true);
  bind->bindNew("autoLateralMax", _autoMaxLateral, RhIO::Bind::PullOnly)->defaultValue(0.02)->persisted(true);
  bind->bindNew("autoTurnMin", _autoMinTurn, RhIO::Bind::PullOnly)->defaultValue(-0.3)->persisted(true);
  bind->bindNew("autoTurnMax", _autoMaxTurn, RhIO::Bind::PullOnly)->defaultValue(0.3)->persisted(true);
  bind->bindNew("autoStepDelta", _autoDeltaStep, RhIO::Bind::PullOnly)->defaultValue(0.01)->persisted(true);
  bind->bindNew("autoLateralDelta", _autoDeltaLateral, RhIO::Bind::PullOnly)->defaultValue(0.01)->persisted(true);
  bind->bindNew("autoTurnDelta", _autoDeltaTurn, RhIO::Bind::PullOnly)->defaultValue(0.1)->persisted(true);
...
void OdometryCalibration::step(float elapsed)
{
  ...
  if (_dumpFileName != "")
  {
    ...
    for (size_t i = 0; i < this->_readTrajsPose.size(); i++)
    {
      ...
      {
        file << i << " " << j << " " << _readTrajsPose[i][j].x() << " " << _readTrajsPose[i][j].y() << " "
             << _readTrajsPose[i][j].z() << " " << (int)_readTrajsSupport[i][j] << " " << _goalTrajsPose[i][j].x()
             << " " << _goalTrajsPose[i][j].y() << " " << _goalTrajsPose[i][j].z() << " "
             << (int)_goalTrajsSupport[i][j] << " " << _walkTrajsOrder[i][j].x() << " " << _walkTrajsOrder[i][j].y()
             << " " << _walkTrajsOrder[i][j].z() << " " << _walkTrajsOrder[i][j](3) << " " << _walkTrajsPhase[i][j]
             << " " << _targetDisplacements[i].x() << " " << _targetDisplacements[i].y() << " "
             << _targetDisplacements[i].z() << std::endl;
      }
    }
    file.close();
    _dumpFileName = "";
  ...
  if (_autoEnable && _autoLastPhase > 0.8 && _walk->getPhase() < 0.2)
  {
    ...
    _randomWalk.uniformStepWithBounds(deltaLow, deltaUp, stateLow, stateUp, _autoInertia);
    _cycleCounts++;
    // If number of cycles is finished, the
    if (_cycleCounts <= _autoNbCycles)
    {
      _walk->setRawOrder(_randomWalk.statePos().x(), _randomWalk.statePos().y(), _randomWalk.statePos().z(), true);
    }
    else
    {
      _isLogging = false;
...
std::string OdometryCalibration::cmdReferenceDisplacement(double dx, double dy, double da, bool setLast)
{
  std::lock_guard<std::mutex> lock(_mutex);
  _displacement.x() = dx;
  _displacement.y() = dy;
  ...
  return "Set target reference relative displacement to:" + std::string(" dX=") + std::to_string(dx) +
         std::string(" dY=") + std::to_string(dy) + std::string(" dA=") + std::to_string(da);
}
...
std::string OdometryCalibration::cmdStartLog()
{
  std::lock_guard<std::mutex> lock(_mutex);
  _isLogging = true;
  _readTrajsPose.push_back(std::vector<Eigen::Vector3d>());
  _readTrajsSupport.push_back(std::vector<Leph::HumanoidFixedModel::SupportFoot>());
  _goalTrajsPose.push_back(std::vector<Eigen::Vector3d>());
  _goalTrajsSupport.push_back(std::vector<Leph::HumanoidFixedModel::SupportFoot>());
  _walkTrajsOrder.push_back(std::vector<Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d>>());
  _walkTrajsPhase.push_back(std::vector<double>());
  ...
  return "Starting new learning sequence with target displacement: " + std::string(" dX=") +
         std::to_string(_displacement.x()) + std::string(" dY=") + std::to_string(_displacement.y()) +
...
std::string OdometryCalibration::cmdStopLog()
{
  ...
  return "Stopping learning sequence " + std::to_string(_readTrajsPose.size()) + std::string(" length=") +
...
std::string OdometryCalibration::cmdCancelLog()
{
  ...
  return "Cancelling learning sequence " + std::to_string(_readTrajsPose.size()) + std::string(" length=") +
         std::to_string(_readTrajsPose.back().size());
}
