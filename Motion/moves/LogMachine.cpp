...
LogMachine::LogMachine(Walk* walk_, Head* head_) : walk(walk_), head(head_), status(""), dumpFileName(""), randomWalk(3)
{
  ...
  bind->bindFunc("log_machine_set_last_pos",
                 "Set target reference displacement <dX> <dY> <dA> in source frame." + std::string("Assign given "
                                                                                                   "position to last "
                                                                                                   "sequence"),
                 &LogMachine::cmdSetLastRunPos, *this, { "", "", "" });
  bind->bindFunc("log_machine_start_sequence", "Start a multiple run logs sequence <nbRuns>",
                 &LogMachine::cmdStartSequence, *this);
  bind->bindFunc("log_machine_start", "Start logging a run", &LogMachine::cmdStartLog, *this);
  bind->bindFunc("log_machine_stop", "Stop logging current run", &LogMachine::cmdStopLog, *this);
  bind->bindFunc("log_machine_cancel", "Stop logging and pop last started learning sequence.",
                 &LogMachine::cmdCancelLog, *this);
  bind->bindFunc("log_machine_dump", "Dump recorded odometry data into given filename", &LogMachine::cmdDumpData,
                 *this);
  bind->bindFunc("log_machine_clear", "Remove all collected logs from the current list", &LogMachine::cmdClear, *this);
  ...
  bind->bindNew("time", time, RhIO::Bind::PushOnly)->defaultValue(0)->comment("Time spent in current phase [s]?");
  ...
  bind->bindNew("cmdStep", cmdStep, RhIO::Bind::PullOnly)->defaultValue(0)->comment("Size of forward step [m]");
  bind->bindNew("cmdLat", cmdLat, RhIO::Bind::PullOnly)->defaultValue(0)->comment("Size of lateral step [m]");
  bind->bindNew("cmdRot", cmdRot, RhIO::Bind::PullOnly)->defaultValue(0)->comment("Rotation at each step [rad]");
...
void LogMachine::step(float elapsed)
{
  ...
  else if (state != STATE_INCAPACITED && incapacited)
  {
    ...
    {
      logger.log("Failed log: isFallen -> %d and handled -> %d", decision->isFallen, decision->handled);
      endRun();
  ...
  else if (state == STATE_EPILOGUE && localized)
  {
    ...
    logger.log("Ending log with delta: x: %lf [m], y: %lf [m], theta: %lf [deg]", dx, dy, rad2deg(dtheta));
    sequencesMutex.lock();
  ...
  else if (state == STATE_LOGGING)
  {
    ...
    {
      sequencesMutex.lock();
      sequences.back().pushEntry(time, stepCount, service->readModel(), service->goalModel(), walk->getOrder(),
                                 walk->getPhase());
      sequencesMutex.unlock();
    }
    ...
    if (autoMode && walk->isNewStep())
    {
      ...
      if (!fixedSpeed && !customOrders)
      {
        ...
        // Applying randomWalk
        randomWalk.uniformStepWithBounds(deltaLimits.col(0), deltaLimits.col(1), stateLimits.col(0), stateLimits.col(1),
                                         0);
        std::cout << "RandomWalk: " << randomWalk.statePos().transpose() << std::endl;
...
void LogMachine::startRun()
{
  ...
  if (customOrders)
  {
    ...
    logger.log("Starting log at custom speed: %f[m/step], %f[m/step], %f[rad/step]", autoSpeed(0), autoSpeed(1),
               autoSpeed(2));
  }
  else if (fixedSpeed)
  {
    autoSpeed = getRandomSpeed();
    logger.log("Starting log at fixed speed: %f[m/step], %f[m/step], %f[rad/step]", autoSpeed(0), autoSpeed(1),
               autoSpeed(2));
  }
  else
  {
...
std::string LogMachine::cmdStartSequence(int nbRuns)
{
  ...
  nbRunsLeft = nbRuns;
  setState(STATE_PREAMBLE);
  return "Started an auto log sequence of " + std::to_string(nbRuns) + " runs!" + " DID YOU THINK ABOUT GYROTARE ???";
}
...
std::string LogMachine::cmdStopLog()
{
  ...
  oss << "Learning sequence stopped: " << stepCount << " steps in " << time << " ms ("
