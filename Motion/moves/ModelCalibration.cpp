 ...
 * static position
 */
static Eigen::Vector3d staticTrunkPos(-0.00557785331559037, -0.0115849568418458, 0.285);
static Eigen::Vector3d staticTrunkAxis(-0.672036398746933, 0.0743358280850477, 0.0028323027017884);
static Eigen::Vector3d staticFootPos(0.0208647084129351, -0.095, 0.0591693358237435);
static Eigen::Vector3d staticFootAxis(0.0, 0.0, 0.0);
static Leph::HumanoidFixedModel::SupportFoot staticSupport(Leph::HumanoidFixedModel::LeftSupportFoot);
  ...
  , _trajs()
{
  ...
  // Binding
  bind->bindNew("savePrefix", _logPrefix, RhIO::Bind::PullOnly)->comment("Log prefix path")->defaultValue(_logPrefix);
  ...
  // Bind command
  bind->bindFunc("calibration_model_start", "Start given calibration sequence", &ModelCalibration::cmdStart, *this);
}
...
void ModelCalibration::step(float elapsed)
{
  ...
  bool isSuccess = goalModel.trunkFootIK(staticSupport, staticTrunkPos, Leph::AxisToMatrix(staticTrunkAxis),
                                         staticFootPos, Leph::AxisToMatrix(staticFootAxis));
  if (!isSuccess)
  {
    _errorMsg = "IK Error at time=" + std::to_string(_motionTime);
  }
  ...
  else
  {
    ...
    if (_motionTime > -1.5 && _motionTime < 0.0)
    {
      ...
      service->startLogging(
          _logPrefix + _motionName + std::string("_") + Leph::currentDate() + std::string(".mapseries"), false);
    ...
    if (_motionName == "shoulder_pitch")
    {
      ...
      if (_motionIsSmooth)
      {
        spline = buildSmoothTraj(_motionAngleStart, _motionPeriod, _motionAngleAmpl * M_PI / 180.0, false);
      }
      else
      {
        spline = buildStepTraj(_motionAngleStart, _motionPeriod, _motionAngleAmpl * M_PI / 180.0, false);
      }
      endTime = spline.max();
    ...
    else if (_motionName == "foot_x")
    {
      ...
      if (_motionIsSmooth)
      {
        spline = buildSmoothTraj(_motionCartStart, _motionPeriod, _motionCartAmpl, false);
      }
      else
      {
        spline = buildStepTraj(_motionCartStart, _motionPeriod, _motionCartAmpl, false);
      }
      endTime = spline.max();
      bool isSuccess = goalModel.trunkFootIK(staticSupport, staticTrunkPos, Leph::AxisToMatrix(staticTrunkAxis),
    ...
    else if (_motionName == "trunk_x")
    {
      ...
      endTime = spline.max();
      bool isSuccess =
          goalModel.trunkFootIK(staticSupport, staticTrunkPos + Eigen::Vector3d(spline.pos(time), 0.0, 0.0),
                                Leph::AxisToMatrix(staticTrunkAxis), staticFootPos, Leph::AxisToMatrix(staticFootAxis));
      if (!isSuccess)
      {
        _errorMsg = "IK Error at time=" + std::to_string(_motionTime);
      }
    ...
    else if (_motionName == "trunk_y")
    {
      ...
      endTime = spline.max();
      bool isSuccess =
          goalModel.trunkFootIK(staticSupport, staticTrunkPos + Eigen::Vector3d(0.0, spline.pos(time), 0.0),
                                Leph::AxisToMatrix(staticTrunkAxis), staticFootPos, Leph::AxisToMatrix(staticFootAxis));
      if (!isSuccess)
      {
        _errorMsg = "IK Error at time=" + std::to_string(_motionTime);
    ...
    else if (_motionName == "trunk_z")
    {
      ...
      endTime = spline.max();
      bool isSuccess =
          goalModel.trunkFootIK(staticSupport, staticTrunkPos + Eigen::Vector3d(0.0, 0.0, spline.pos(time)),
                                Leph::AxisToMatrix(staticTrunkAxis), staticFootPos, Leph::AxisToMatrix(staticFootAxis));
      if (!isSuccess)
      {
        _errorMsg = "IK Error at time=" + std::to_string(_motionTime);
    ...
    else if (_motionName == "trajectory")
    {
      ...
      Leph::TrajectoriesTrunkFootPos(time, _trajs, trunkPos, trunkAxis, footPos, footAxis);
      Leph::TrajectoriesSupportFootState(time, _trajs, isDoubleSupport, supportFoot);
      endTime = _trajs.max();
      // Run inverse kinematics
      bool isSuccess = goalModel.trunkFootIK(supportFoot, trunkPos, Leph::AxisToMatrix(trunkAxis), footPos,
    ...
    // Update motion state
    if (_motionName != "" && _motionTime >= 0.0)
    {
      // Increase playing time
      _motionTime += elapsed;
    }
    // Motion end
    if (_motionName != "" && _motionTime >= 0.0 && _motionTime >= endTime)
    {
      // Stop log and wait
      service->stopLogging();
      _countWait = 100;
...
Leph::Spline ModelCalibration::buildStepTraj(double start, double period, double ampl, bool isHalf)
{
...
Leph::Spline ModelCalibration::buildSmoothTraj(double start, double period, double ampl, bool isHalf)
{
  // Time delay before and after moving
  double pauseTime = 1.0;
...
std::string ModelCalibration::cmdStart(std::string name)
{
  ...
  _motionTime = -2.0;
  _mutex.unlock();
  return "Start calibration sequence: " + name + " with prefix: " + _logPrefix;
}
