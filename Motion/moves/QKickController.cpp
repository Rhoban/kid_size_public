...
QKickController::QKickController()
{
  ...
  bind->bindNew("farUpdateDist", farUpdateDist, RhIO::Bind::PullOnly)->defaultValue(0.35)->persisted(true);

  bind->bindNew("moveUpdateDist", moveUpdateDist, RhIO::Bind::PullOnly)->defaultValue(0.5)->persisted(true);

  bind->bindFunc("reloadStrategy", "Reload the strategy file", &QKickController::cmdReloadStrategy, *this);
...
void QKickController::updateAction()
{
  ...
  if (avoidOpponents)
  {
    ...
    if (opponentsPos.size())
    {
      ...
      auto tmp = kmc.getKickModel(kick).applyKick(Eigen::Vector2d(ball.x, ball.y), action.orientation);
        ...
        */
        ...
        for (auto& tangent : tangents)
        {
          ...
          for (auto& kickName : kmc.getKickNames())
          {
            if (kickName == "small")
              continue;
            auto targetRes = kmc.getKickModel(kickName).applyKick(Eigen::Vector2d(ball.x, ball.y), orientation);
