                ...
                for (auto &tangent : tangents) {
                    ...
                    for (auto &kickName : kmc.getKickNames()) {
                        ...
                        if (intersects(target)) {
                            ...
                            ok = false;
                        } else if (fabs(target.x) > Constants::field.fieldLength/200.0 ||
                                fabs(target.y) > Constants::field.fieldWidth/200.0) {
                            // Ball goes out of the field
                            ok = false;
                            if (target.x > Constants::field.fieldLength/200.0 &&
                                    ball.x <= Constants::field.fieldLength/200.0) {
                                double dY = (target.y-ball.y)/(target.x-ball.x);
                                double yIntersect = ball.y + (Constants::field.fieldLength/200.0-ball.x)*dY;

                                if (fabs(yIntersect) < Constants::field.goalWidth*0.8/200.0) {
