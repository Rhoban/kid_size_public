...
Placer::Placer(Walk* walk) : walk(walk)
{
  initializeBinding();
  // Anti-SigFPE
  bind->pull();
  ...
  bind->bindNew("pathTargetDist", pathTargetDist, RhIO::Bind::PullOnly)->defaultValue(0.7);
  bind->bindNew("tmpX", tmpX, RhIO::Bind::PushOnly);
  bind->bindNew("tmpY", tmpY, RhIO::Bind::PushOnly);

  bind->bindNew("targetX", targetX, RhIO::Bind::PullOnly)->comment("[m]")->defaultValue(0.0);
  bind->bindNew("errorX", errorX, RhIO::Bind::PushOnly)->comment("[m]")->defaultValue(0.0);
  bind->bindNew("targetY", targetY, RhIO::Bind::PullOnly)->comment("[m]")->defaultValue(0.0);
  bind->bindNew("errorY", errorY, RhIO::Bind::PushOnly)->comment("[m]")->defaultValue(0.0);
  ...
  bind->bindNew("directMode", directMode, RhIO::Bind::PullOnly)
      ->defaultValue(true)
      ->comment("Turning to target before walking target");
  bind->bindNew("rushDistOk", rushDistOk, RhIO::Bind::PushOnly)
      ->defaultValue(false)
      ->comment("Is the distance to the target suggesting to rush");
  bind->bindNew("rushAngleOk", rushAngleOk, RhIO::Bind::PushOnly)
      ->defaultValue(false)
      ->comment("Is the angle toward the target valid for rush");
  ...
  bind->bindNew("capToTarget", capToTarget, RhIO::Bind::PushOnly)
      ->defaultValue(0)
      ->comment("The direction of the target in the robot referential");
  ...
  bind->bindNew("avoidMates", avoidMates, RhIO::Bind::PullOnly)
      ->defaultValue(true)
      ->comment("Avoid the team mates based on localisation?");
  bind->bindNew("avoidSharedOpponents", avoidSharedOpponents, RhIO::Bind::PullOnly)
      ->defaultValue(false)
      ->comment("Avoid the shared opponents?");
  bind->pull();
}
...
void Placer::goTo(float x, float y, float azimuth, std::vector<Circle> obstacles_)
{
  // logger.log("goTo: %f (%f/%f) , %f (%f/%f) %f (%f/%f)",x,
  // errorX,marginX,y,errorY,marginY,azimuth,errorAzimuth,marginAzimuth);
...
void Placer::step(float elapsed)
{
  ...
  if (arrived)
  {
    ...
    {
      logger.log("I am too far from destination, I'm on my way: errorX: %f, errorY: %f", errorX, errorY);
      arrived = false;
    }
  }
  else
  {
    ...
    {
      logger.log("I consider that I reached destination: errorX: %f, errorY: %f, errorAzimuth %f", errorX, errorY,
                 errorAzimuth);
      arrived = true;
    }
  }
  ...
  auto result = avoider.findPath(pos, target, 0.1, &score, [](Point pt) {
    return fabs(pt.x) < (Constants::field.field_length / 2 + 0.5) &&
           fabs(pt.y) < (Constants::field.field_width / 2 + 0.5);
  });
  ...
  if (!arrived)
  {
    ...
    if (directMode)
    {
      ...
      if (!rushDistOk)
      {  // Use classic mode for short range
        stepper.update(vecX);
        lateraler.update(vecY);
        turner.update(vecAzimuth);
      }
      else if (!rushAngleOk)
      {  // rush is not allowed, align to target first
        lateraler.update(0);
        stepper.update(0);
        turner.update(vecCap);
      }
      else
      {  // rush toward the target while aligning toward it
        stepper.update(vecX);
        lateraler.update(0);
        turner.update(vecCap);
      }
