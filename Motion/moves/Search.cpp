...
Search::Search(Walk* walk, Placer* placer) : walk(walk), placer(placer)
{
  initializeBinding();
  // State
  bind->bindNew("state", STM::state, RhIO::Bind::PushOnly)->comment("Search STM state");
  ...
  bind->bindNew("beginX", beginX, RhIO::Bind::PullOnly)->comment("Begin X")->defaultValue(0)->persisted(true);
  bind->bindNew("beginY", beginY, RhIO::Bind::PullOnly)->comment("Begin Y")->defaultValue(0)->persisted(true);
  bind->bindNew("beginAzimuth", beginAzimuth, RhIO::Bind::PullOnly)
      ->comment("Begin Azimuth")
      ->defaultValue(0)
      ->persisted(true);

  bind->bindNew("P1X", P1X, RhIO::Bind::PullOnly)->comment("Patrol checkpoint 1 X")->defaultValue(0);
  bind->bindNew("P1Y", P1Y, RhIO::Bind::PullOnly)->comment("Patrol checkpoint 1 Y")->defaultValue(0);
  bind->bindNew("P1Azimuth", P1Azimuth, RhIO::Bind::PullOnly)->comment("Patrol checkpoint 1 Azimuth")->defaultValue(0);

  bind->bindNew("P2X", P2X, RhIO::Bind::PullOnly)->comment("Patrol checkpoint 2 X")->defaultValue(0);
  bind->bindNew("P2Y", P2Y, RhIO::Bind::PullOnly)->comment("Patrol checkpoint 2 Y")->defaultValue(0);
  bind->bindNew("P2Azimuth", P2Azimuth, RhIO::Bind::PullOnly)->comment("Patrol checkpoint 2 Azimuth")->defaultValue(0);
  ...
  bind->bindNew("goLeft", goLeft)->defaultValue(false);
  bind->bindNew("goRight", goRight)->defaultValue(false);
  bind->bindNew("goForward", goForward)->defaultValue(false);
