...
LearnedApproach::LearnedApproach(Walk* walk_) : ApproachMove(walk_)
{
  ...
  // Binding commands
  bind->bindFunc("learned_approach_update_policies", "Update the policies used for learned_approach",
                 &LearnedApproach::updatePolicies, *this);
  bind->bindFunc("learned_approach_update_problems", "Update the problems used for learned_approach",
                 &LearnedApproach::updateProblems, *this);
  // Bind members
  bind->bindNew("time", time, RhIO::Bind::PushOnly)->defaultValue(-1.0);
  bind->bindNew("log_file", log_file, RhIO::Bind::PullOnly)->defaultValue("");
  // Kick related variables
  bind->bindNew("doKick", do_kick, RhIO::Bind::PullOnly)
      ->defaultValue(true)
      ->comment("Start a kick when the approach is finish")
      ->persisted(true);
  bind->bindNew("warming", is_warming_up, RhIO::Bind::PushOnly)
      ->defaultValue(false)
      ->comment("Is the robot warming up to prepare shoot");
  bind->bindNew("warm_up_time", warm_up_time, RhIO::Bind::PullOnly)
      ->defaultValue(1.0)
  ...
  {
    bind->bindNew(entry.first + "_policy_file", entry.second.policy_file, RhIO::Bind::PullOnly)
        ->defaultValue("learned_approach/" + entry.first + "/policy.json")
        ->persisted(true);
    bind->bindNew(entry.first + "_problem_file", entry.second.problem_file, RhIO::Bind::PullOnly)
...
void LearnedApproach::onStart()
{
  ...
  for (const auto& entry : kick_solutions)
  {
    ...
    {
      logger.error("Approach failed to start: no policy loaded for '%s'", entry.first.c_str());
      onStop();
  ...
  if (log_file != "")
  {
    ...
    if (!file.is_open())
    {
      throw std::runtime_error("LearnedApproach unable to open log: " + log_file);
    }
    file.close();
  }
}
...
void LearnedApproach::step(float elapsed)
{
  ...
  {
    logger.error("launching move while no policy is loaded for kick '%s'", active_approach.c_str());
    stop();
  }
...
void LearnedApproach::updateWalk(const Eigen::VectorXd& state)
{
  ...
  if (log_file != "" && walk->isNewStep())
  {
    ...
    file << state(0) << "," << state(1) << "," << state(2) << "," << state(3) << "," << state(4) << "," << state(5)
         << "," << new_orders(0) << "," << new_orders(1) << "," << new_orders(2) << std::endl;
    file.close();
  }
}
...
void LearnedApproach::updatePolicies()
{
  ...
  for (auto& entry : kick_solutions)
  {
    ...
    logger.log("Loading policy '%s' at '%s'", entry.first.c_str(), path.c_str());
    try
    {
      ks.policy = PolicyFactory().buildFromJsonFile(path);
...
void LearnedApproach::updateProblems()
{
  ...
  for (auto& entry : kick_solutions)
  {
    KickSolution& ks = entry.second;
    std::string path = ks.policy_file;
    logger.log("Loading problem '%s' at '%s'", entry.first.c_str(), path.c_str());
    try
    {
...
void LearnedApproach::displayProblemLimits()
{
  for (const auto& entry : kick_solutions)
  {
    ...
    {
      std::cout << "-> " << action_id << ": " << std::endl << ks.problem.getActionLimits(action_id) << std::endl;
    }
  }
}
