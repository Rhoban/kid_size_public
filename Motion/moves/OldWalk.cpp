...
OldWalk::OldWalk(Kick* kickMove) : kickMove(kickMove), ratioHistory(0.5)
{
  ...
  RhIO::Root.newCommand("moves/walk/walkKick", "request the walk to perform a kick",
  ...
  // Tresholds
  bind->bindNew("securityTreshold", securityTreshold)->defaultValue(0.065)->minimum(0)->maximum(1.0)->persisted(true);
  bind->bindNew("securityPhase", securityPhase)->defaultValue(0.1)->persisted(true);
  // Foot roll for compensation
  bind->bindNew("compensateRoll", compensateRoll)->defaultValue(0.0)->minimum(-45)->maximum(45)->persisted(true);
  leftRoll = 0;
  rightRoll = 0;

  // Shooting
  bind->bindNew("shootPhase", shootPhase, RhIO::Bind::PullOnly)
      ->comment("Shooting step")
      ->defaultValue(0)
      ->persisted(true);
  ...
  // Trimming
  bind->bindNew("stepTrim", stepTrim, RhIO::Bind::PullOnly)->defaultValue(0.0)->persisted(true);
  bind->bindNew("lateralTrim", lateralTrim, RhIO::Bind::PullOnly)->defaultValue(0.0)->persisted(true);
  bind->bindNew("turnTrim", turnTrim, RhIO::Bind::PullOnly)->defaultValue(0.0)->persisted(true);
  ...
  bind->bindNew("P_stepXOffset", P_stepXOffset, RhIO::Bind::PullOnly)
      ->comment("P factor on trunkXOffset related to step")
      ->persisted(true)
      ->defaultValue(0);

  bind->bindNew("P_stepPitch", P_stepPitch, RhIO::Bind::PullOnly)
      ->comment("P factor on trunkPitch related to step")
      ->persisted(true)
      ->defaultValue(0);

  bind->bindNew("P_stepXOffset_back", P_stepXOffset_back, RhIO::Bind::PullOnly)
      ->comment("P factor on trunkXOffset related to step (backward)")
      ->persisted(true)
      ->defaultValue(0);

  bind->bindNew("P_stepPitch_back", P_stepPitch_back, RhIO::Bind::PullOnly)
      ->comment("P factor on trunkPitch related to step (backward)")
      ->persisted(true)
      ->defaultValue(0);

  bind->bindNew("smoothTransition", smoothTransition, RhIO::Bind::PullOnly)
      ->comment("Smoothing factor when changing parameters")
      ->persisted(true)
      ->defaultValue(0.5)
      ->minimum(0)
      ->maximum(1);
  ...
  bind->bindNew("smoothCommands", smoothCommands, RhIO::Bind::PullOnly)
      ->comment("Maximum step acceleration")
      ->persisted(true)
      ->defaultValue(0.9)
      ->minimum(0)
      ->maximum(1);
  ...
  // Walk control
  bind->bindNew("walkEnable", walkEnable, RhIO::Bind::PullOnly)->comment("Walk control Enable")->defaultValue(false);
  ...
  // Kicks
  bind->bindNew("walkKickLeft", walkKickLeft, RhIO::Bind::PullOnly)->comment("Kick left")->defaultValue(false);
  bind->bindNew("walkKickRight", walkKickRight, RhIO::Bind::PullOnly)->comment("Kick right")->defaultValue(false);
  bind->bindNew("walkKickName", walkKickName, RhIO::Bind::PullOnly)->comment("Kick name")->defaultValue("classic");

  // Last step variables
  bind->node().newInt("lastStepAsk")->comment("0: no last step. 1: left foot. 2: right foot.")->defaultValue(0);
  bind->node().newFloat("lastStepForward")->defaultValue(0.0);
  bind->node().newFloat("lastStepLateral")->defaultValue(0.0);
  bind->node().newFloat("lastStepTurn")->defaultValue(0.0);
  ...
  bind->bindNew("cooldown", cooldown, RhIO::Bind::PullOnly)
      ...
      ->persisted(false);
  bind->bindNew("warmup", warmup, RhIO::Bind::PullOnly)->defaultValue(0.75)->comment("Warmup [s]")->persisted(false);

  bind->bindNew("paramsStepGain", _orders.x(), RhIO::Bind::PushOnly)->comment("IKWalk really used step parameters");
  bind->bindNew("paramsLateralGain", _orders.y(), RhIO::Bind::PushOnly)
      ->comment("IKWalk really used lateral parameters");
  bind->bindNew("paramsTurnGain", _orders.z(), RhIO::Bind::PushOnly)->comment("IKWalk really used turn parameters");

  bind->bindNew("pressureYStdThresholdWarmup", pressureYStdThresholdWarmup, RhIO::Bind::PullOnly)->defaultValue(0.04);
  ...
  // Speed limits
  bind->bindNew("maxRotation", maxRotation, RhIO::Bind::PullOnly)->defaultValue(12.0);

  bind->bindNew("maxStep", maxStep, RhIO::Bind::PullOnly)->defaultValue(40.0);

  bind->bindNew("maxStepBackward", maxStepBackward, RhIO::Bind::PullOnly)->defaultValue(20.0);
  ...
  bind->bindNew("gkMustRaise", gkMustRaise, RhIO::Bind::PullOnly)
      ->comment("goal keeper flag: walk must raise")
      ->defaultValue(false);
  bind->bindNew("gkMustBlock", gkMustBlock, RhIO::Bind::PullOnly)
      ->comment("goal keeper flag: walk must be blocked")
      ->defaultValue(false);

#ifdef MODE_NOMINAL
  bind->node().newFloat("guessPhase");
  bind->node().newFloat("nominalScore");
  bind->node().newFloat("phaseMaxV")->defaultValue(0.1);
...
double OldWalk::getPhase() const
{
  return phase;
}

double OldWalk::getLastPhase() const
{
  return lastPhase;
}

bool OldWalk::isNewStep(double elapsed) const
{
...
Eigen::Vector4d OldWalk::getRawOrder() const
{
  return Eigen::Vector4d(_orders.x(), _orders.y(), _orders.z(), _isEnabled);
}

Eigen::Vector4d OldWalk::getOrder() const
{
  return Eigen::Vector4d(smoothingStep / 1000, smoothingLateral / 1000, deg2rad(smoothingTurn), smoothing);
}

void OldWalk::setRawOrder(double step, double lateral, double turn, bool enable)
{
...
void OldWalk::setRawOrder(const Eigen::Vector3d& params, bool enable)
{
  setRawOrder(params(0), params(1), params(2), enable);
}

Eigen::Vector3d OldWalk::getMinOrders() const
{
...
Eigen::Vector3d OldWalk::getMaxOrders() const
{
  Eigen::Vector3d bound;
...
Eigen::Vector3d OldWalk::getMinDeltaOrders() const
{
  Eigen::Vector3d bound;
...
Eigen::Vector3d OldWalk::getMaxDeltaOrders() const
{
  Eigen::Vector3d bound;
  bound << maxDStepByCycle / 1000, maxDLatByCycle / 1000, deg2rad(maxDTurnByCycle);
  return bound;
...
void OldWalk::askLastStep(const Eigen::Vector3d& deltaPose)
{
...
void OldWalk::control(bool enable, double step, double lateral, double turn)
{
  if (isRunning())
  {
    if (dontWalk)
...
void OldWalk::kick(bool rightFoot, const std::string& kickName)
{
...
bool OldWalk::isKicking()
{
  return shouldKickLeft || shouldKickRight || shootingLeft || shootingRight;
}

bool OldWalk::isLastStep() const
{
  return (_lastStepPhase >= 0.0) || (bind->node().getInt("lastStepAsk") != 0);
}

void OldWalk::onStart()
{
  // Compute trunk height on all DOF set to zero
  Eigen::VectorXd cpyDOF = Helpers::getServices()->model->goalModel().get().getDOFVect();
  Helpers::getServices()->model->goalModel().get().setDOFZeros();
  _trunkHeight = Helpers::getServices()->model->goalModel().get().position("trunk", "left_foot_tip").z();
  _footDistance = Helpers::getServices()->model->goalModel().get().position("left_foot_tip", "right_foot_tip").y();
...
void OldWalk::updateParams(double factor, float step, float lateral, float turn)
{
  ...
  _params("footDistance") = 2.0 * _footYOffset + _footDistance;
  _params("trunkSwing") =
      (factor * _params("trunkSwing") * std::fabs(_params("footDistance")) + (1 - factor) * targetSwingGain) /
      std::fabs(_params("footDistance"));
...
void OldWalk::step(float elapsed)
{
  static float t;
  ...
  if (!forbidOrders && newStep)
  {
    ...
    if (fabs(smoothingStep) > fabs(step)*3)
      tmpMaxDStepByCycle *= 5;
    if (fabs(smoothingLateral) > fabs(lateral)*3)
      tmpMaxDLatByCycle *= 5;
    if (fabs(smoothingTurn) > fabs(turn)*3)
  ...
  if (walkEnable != walkEnableTarget)
  {
    ...
    if ((!walkEnable && walkEnableTimeSinceChange > 0.3) || (walkEnable && isStableCooldown))
    {
  ...
  if (_lastStepPhase >= 0.0)
  {
    ...
    if (_lastStepCount % 2 == 0 && ((_lastStepPhase >= 0.0 && _lastStepPhase < 0.5 && _engine.getPhase() > 0.5) ||
                                    (_lastStepPhase >= 0.5 && _lastStepPhase < 1.0 && _engine.getPhase() < 0.5)))
    {
      _lastStepCount++;
    }
    if (_lastStepCount % 2 == 1 && ((_lastStepPhase >= 0.0 && _lastStepPhase < 0.5 && _engine.getPhase() < 0.5) ||
                                    (_lastStepPhase >= 0.5 && _lastStepPhase < 1.0 && _engine.getPhase() > 0.5)))
    {
      _lastStepCount++;
    }
    // Retrieve last step displacement
    Eigen::Vector3d lastOrder(bind->node().getFloat("lastStepForward"), bind->node().getFloat("lastStepLateral"),
...
void OldWalk::updateShoot(bool left, const std::string& kickName)
{
  kickMove->set(left, kickName);
}

void OldWalk::endShoot()
{
  shootingLeft = false;
  shootingRight = false;
}
