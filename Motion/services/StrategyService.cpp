  ...
  , lastKick(TimeStamp::fromMS(0))
{
  ...
  // Kick target
  bind.bindNew("kickTargetX", kick_target_x, RhIO::Bind::PushOnly)->defaultValue(0);
  bind.bindNew("kickTargetY", kick_target_y, RhIO::Bind::PushOnly)->defaultValue(0);
  bind.bindNew("kickControllerDir", kick_controler_dir, RhIO::Bind::PushOnly)->defaultValue(0);
  // Kick tolerance
  bind.bindNew("kickTolerance", kick_tolerance, RhIO::Bind::PushOnly)->defaultValue(0);
}
...
template <class T>
std::string getActiveMove(const std::vector<std::string>& candidates, MoveScheduler* scheduler)
{
  ...
  if (active_moves.size() == 0)
    return "none";
  if (active_moves.size() > 1)
    return "conflict";
...
bool StrategyService::tick(double elapsed)
{
  ...
  static std::vector<std::string> approach_candidates = { "approach", "approach_potential" };
  static std::vector<std::string> kick_controler_candidates = { "q_kick_controler", "clearing_kick_controler",
                                                                "penalty_kick_controler" };

  bind.pull();

  active_approach = getActiveMove<ApproachMove*>(approach_candidates, getScheduler());
  active_kick_controler = getActiveMove<KickController*>(kick_controler_candidates, getScheduler());
