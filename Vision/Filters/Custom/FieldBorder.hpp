...
public:
  ...
  void update_loc_data(std::vector<cv::Vec2f> line_pair_eq, std::vector<std::pair<int, int> > line_pair_domain,
                       std::vector<double> line_quality, double X, bool final_compute);
  void compute_marks(cv::Mat& green_density, std::vector<cv::Point>& marks, std::vector<double>& threshold);
  void line_detection(std::vector<cv::Point>& marks, std::vector<cv::Vec2f>& brut_lines,
  ...
  void filter_bad_marks(std::vector<cv::Point>& marks, std::vector<cv::Point>& bad_marks,
                        std::vector<cv::Vec2f>& line_eq, int pix_precision);

  void compute_precision_scores(std::vector<cv::Point>& marks, std::vector<cv::Vec2f>& line_eq,
                                std::vector<std::pair<int, int> >& line_x_domain);

  void compute_geometric_score(std::map<std::pair<int, int>, std::pair<double, double> >& line_pairs_geom_scores,
                               std::map<std::pair<int, int>, int>& line_pairs_X_inter, std::vector<cv::Vec2f>& line_eq,
                               std::vector<std::pair<int, int> >& line_x_domain);

  void compute_separation_score(cv::Mat& green_density, std::vector<double>& threshold, std::vector<cv::Vec2f>& line_eq,
                                std::vector<std::pair<int, int> >& line_x_domain);

  void compute_border(std::vector<cv::Point>& marks, std::vector<cv::Vec2f>& line_eq,
                      std::vector<std::pair<int, int> >& line_x_domain,
                      std::map<std::pair<int, int>, int>& line_pairs_X_inter,
                      std::map<std::pair<int, int>, std::pair<double, double> >& line_pairs_geom_scores);

  void debug_tag(cv::Mat* green, cv::Mat* green_density, std::vector<cv::Point>& marks,
