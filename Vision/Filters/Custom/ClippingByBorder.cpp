...
void ClippingByBorder::process()
{
  ...
  {
    if (debug_output)
      printf("ClippingByBorder : I have no data\n");
    img().setTo(255);
    return;
  }
  ...
  {
    if (debug_output)
      printf("ClippingByBorder : bad score\n");
    img().setTo(255);
    return;
  }

  std::vector<std::pair<cv::Point2f, cv::Point2f> > pix_lines = FieldBorder::current_loc_data->getPixLines();
