...
void FieldBorder::process()
{
  ...
  line_detection(marks, brut_lines, line_eq, line_x_domain, hough_threshold, pix_precision, angle_precision);
  ...
  std::map<std::pair<int, int>, std::pair<double, double> > line_pairs_geom_scores;  // (idx,idx) -> (score, confidence)
                            ...
                            std::vector<cv::Vec2f>& line_eq)
{
  ...
  if (tag_level >= 3)
  {
    for (size_t i = 0; i < brut_lines.size(); i++)
    {
      ...
      line(img(), scale_factor * pt1, scale_factor * pt2, cv::Scalar::all(255), 1, CV_AA);
    }
  }
                                  ...
                                  double X, bool final_compute)
{
  ...
  loc_data.pushPixLine(line_quality[0], std::pair<cv::Point2f, cv::Point2f>(Uf, Vf));
  ...
  if (line_pair_eq.size() >= 2)
  {
    ...
    cv::Point2f Uf(((float)U.x) / col_nb, ((float)U.y) / row_nb);
    cv::Point2f Vf(((float)V.x) / col_nb, ((float)V.y) / row_nb);
    loc_data.pushPixLine(line_quality[1], std::pair<cv::Point2f, cv::Point2f>(Uf, Vf));
                                 ...
                                 std::map<std::pair<int, int>, std::pair<double, double> >& line_pairs_geom_scores)
{
  ...
  for (int ll = 0; ll < (int)line_eq.size(); ll++)
  {
    for (int rl = 0; rl < (int)line_eq.size(); rl++)
    {
      ...
      // l'image intégrale
      float separation_score = lsscore_cum(ll, K) + lsscore_cum(rl, corr_comb_size - 1) - lsscore_cum(rl, K);
      ...
      float score = ((pre_weight * precision_score) + (sep_weight * separation_score) + (geo_weight * geo_score) +
                                           ...
                                           std::vector<std::pair<int, int> >& line_x_domain)
{
  ...
  {  // pour chaque ligne
    ...
    {  // pour chaque colonne du peigne
      ...
      lfscore_cum(l, k) = (k == 0) ? lfscore(l, k) : lfscore_cum(l, k - 1) + lfscore(l, k);
      lfscore_sq_cum(l, k) = (k == 0) ? lfscore_sq(l, k) : lfscore_sq_cum(l, k - 1) + lfscore_sq(l, k);
    }
  }
}
...
bool FieldBorder::allocate_scores(int _line_nb, int _comb_size)
{
  if (_line_nb > line_nb)
  {
    ...
    line_fine_score_square = (float*)malloc(sizeof(float) * _line_nb * _comb_size);
    line_fine_score_cum = (float*)malloc(sizeof(float) * _line_nb * _comb_size);
    line_fine_score_square_cum = (float*)malloc(sizeof(float) * _line_nb * _comb_size);
    line_sep_cum_score = (float*)malloc(sizeof(float) * _line_nb * _comb_size);
