...
private:
  ...
  /// of the expected actualRobotWidth and actualRobotHeight (cm).
  ...
  /// expected robotWidth in cm.
  ParamInt robotWidth;

  /// expected robotWidth in cm (but for real this time, used in the filtering section to delete points
  /// that are inside the same robot).
  ParamInt actualRobotWidth;
  ParamInt actualRobotHeight;
