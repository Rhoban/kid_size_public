...
void FieldBorderData::computeTransformations(Vision::Utils::CameraState* cs, bool final_compute) {
  world_lines.clear();
  self_lines.clear();
  ...
  try {
    ...
    if (hasCorner) {
      ...
      if (self_lines.size() >= 2) {
        ...
        if (CA != 0 && CB != 0) {
          ...
          observation_valid = false;
          if (final_compute) {
            FBPRINT_DEBUG("Segment are too short\n");
          }
        }
      } else {
        hasCorner = false;
        if (final_compute) {
          FBPRINT_DEBUG("There is only one line\n");
        }
      }
    } else {
      if (final_compute) {
        FBPRINT_DEBUG("No corner\n");
      }
      observation_valid = false;
    }
    ...
    // std::cerr << e.what();
    if (final_compute) {
      FBPRINT_DEBUG("CBB: exception during computation\n");
    }
  ...
  if (len > min_len && dist_bot_seg < max_dist) {
    if (debug_info) {
      FBPRINT_DEBUG("Found Segment (%0.2f,%0.2f) -> (%0.2f,%0.2f) (len = %0.2f)\n", self_segment.first.x,
                    self_segment.first.y, self_segment.second.x, self_segment.second.y, len);
    }
    hasSegment = true;
...
std::pair<cv::Point2f, cv::Point2f> FieldBorderData::getSegmentInSelf() const { return self_segment; }

std::pair<cv::Point2f, cv::Point2f> FieldBorderData::getSegmentInWorld() const { return world_segment; }

float FieldBorderData::getRobotCornerDist() { return corner_robot_dist; }
...
cv::Point2f FieldBorderData::getPixCorner() const { return pix_corner; }

cv::Point2f FieldBorderData::getCornerInWorldFrame() const { return world_corner; }

cv::Point2f FieldBorderData::getCornerInSelf() const { return self_corner; }

std::vector<std::pair<cv::Point2f, cv::Point2f> > FieldBorderData::getLinesInSelf() const { return self_lines; }
float FieldBorderData::getCornerAngle() const { return corner_angle; }

std::vector<std::pair<cv::Point2f, cv::Point2f> > FieldBorderData::getLinesInWorldFrame() const { return world_lines; }
...
std::vector<std::pair<cv::Point2f, cv::Point2f> > FieldBorderData::getPixLines() const { return pix_lines; }
...
bool FieldBorderData::is_obs_valid() { return loc_active && observation_valid; }
