...
void FieldBorderData::computeTransformations(Vision::Utils::CameraState* cs, bool final_compute)
{
  ...
  try
  {
    for (size_t k = 0; k < pix_lines.size(); k++)
    {
      ...
      if (line_quality[k] > max_obs_score)
...
std::pair<cv::Point2f, cv::Point2f> FieldBorderData::getSegmentInSelf() const
{
  return self_segment;
}

std::pair<cv::Point2f, cv::Point2f> FieldBorderData::getSegmentInWorld() const
{
