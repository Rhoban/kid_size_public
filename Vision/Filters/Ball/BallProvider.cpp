...
const std::vector<Circle> &BallProvider::getBalls() const { return balls; }

const std::vector<Circle> &BallProvider::getCandidates() const { return candidates; }
...
Circle BallProvider::rescaleCircle(const Circle &circle, const cv::Mat &circle_img) const {
...
Circle BallProvider::scaleToImg(const Circle &circle, const cv::Mat &circle_img) const {
  double new_x, new_y, new_radius;
  new_x = circle.getCenter().x * circle_img.cols / getCS().getCameraModel().getImgWidth();
  new_y = circle.getCenter().y * circle_img.rows / getCS().getCameraModel().getImgHeight();
  new_radius = circle.getRadius() * circle_img.cols;
...
void BallProvider::pushBall(const Circle &circle, const cv::Mat &circle_img) {
  balls.push_back(rescaleCircle(circle, circle_img));
}

void BallProvider::pushBalls(const std::vector<Circle> &circles, const cv::Mat &circle_img) {
  for (const Circle &c : circles) {
    pushBall(c, circle_img);
  }
}

void BallProvider::pushCandidate(const Circle &circle, const cv::Mat &circle_img) {
  candidates.push_back(rescaleCircle(circle, circle_img));
}

void BallProvider::pushCandidates(const std::vector<Circle> &circles, const cv::Mat &circle_img) {
  for (const Circle &c : circles) {
