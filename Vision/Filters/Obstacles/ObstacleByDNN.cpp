...
static void patchToNN(const cv::Mat &patch, double minv, double maxv, int w, int h, tiny_dnn::vec_t &data) {
  ...
  if (res.channels() == 3) {
    ...
    cv::Size sz = res.size();
    int size = sz.height * sz.width;
    for (int i = 0; i < size; i++) data[i] = minv + (maxv - minv) * res.data[i] / 255.0;
  }
}
...
ObstacleByDNN::ObstacleByDNN()
    : ObstacleProvider("ObstacleByDNN"), arch_path("arch.json"), weights_path("weights.data") {}
...
std::string ObstacleByDNN::getClassName() const { return "ObstacleByDNN"; }
...
int ObstacleByDNN::expectedDependencies() const { return 1; }
