...
void drawRotatedRectangles(cv::Mat &img, const std::vector<cv::RotatedRect> &rects, const cv::Scalar &color,
  ...
  for (const cv::RotatedRect &rect : rects) drawRotatedRectangle(img, rect, color, thickness, lineType);
}

double getSmallSide(const cv::RotatedRect &rect) { return std::min(rect.size.width, rect.size.height); }

double getBigSide(const cv::RotatedRect &rect) { return std::max(rect.size.width, rect.size.height); }
