...
cv::Rect cropRect(const cv::Rect &roi, const cv::Mat &img) { return cropRect(roi, img.size()); }
...
cv::Rect cropRect(const cv::Rect &roi, const cv::Size &size) {
  ...
  if (newROI.width >= maxWidth) newROI.width = maxWidth;
  if (newROI.height >= maxHeight) newROI.height = maxHeight;
...
bool isOverlapping(const cv::Rect &r1, const cv::Rect &r2) { return (r1 & r2).area() > 0; }
