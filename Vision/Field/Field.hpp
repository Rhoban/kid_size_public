...
public:
  ...
  static const cv::Point center;
  static cv::Point offset();
  static const double centerRadius;
  static const double fieldLength;
  static const double fieldWidth;
  static const double goalAreaLength;
  static const double goalAreaWidth;
  static const double penaltyMarkDist;
  static const double borderStrip;

  static const double goalWidth; // From one post to another
  ...
  static std::vector<cv::Point> getGoalPosts();
  static std::vector<cv::Point> getArenaCorners();
   ...
   *  1 -> post with y coordinate > 0
   */
  static cv::Point getGoal(int goalNo, int postNo = 0);
  static cv::Point getMyGoal(int postNo = 0);
  static cv::Point getAdvGoal(int postNo = 0);
...
private:
  ...
  void initArena();
  void initCenter();
  void initGoalZone(const cv::Point &init, int dir);
  void initMarks();
  void initGoals();
  void initTags();
  ...
  std::vector<cv::Point> fieldMarks;
  std::vector<Utils::Segment> goals;
  std::vector<cv::Point> arenaCorners;
