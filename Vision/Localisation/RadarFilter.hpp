...
class RadarFilter
{
	...
	virtual void increaseScores(const std::vector<T> &objects)
	{
		...
		for (auto &candidate : candidates) {
			...
			// Same candidate
			candidate.object = candidate.object * (positionDiscount) +
			    object * (1 - positionDiscount);
	...
	virtual void mergeSimilars()
	{
		...
		if (cluster_id == -1) {
		    ...
		    Candidate res;
		    res.score = c.score + cc.score;
		    res.object = (c.object * c.score + cc.object * cc.score) / res.score;
		    clusters[cluster_id] = res;
		}
	    }
