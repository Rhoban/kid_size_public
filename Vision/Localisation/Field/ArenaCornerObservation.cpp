...
ArenaCornerObservation::ArenaCornerObservation(const Vision::Filters::FieldBorderData& brut_data_, double weight_)
{
  ...
  if (brut_data.hasCorner)
  {
    ...
    FieldPosition tr_corner_pos(arenaLength / 2 - bot_in_corner_frm.y, -arenaWidth / 2 + bot_in_corner_frm.x,
                                (bot_orient + Angle(90)).getSignedValue());
    candidates.push_back(tr_corner_pos);
    FieldPosition tl_corner_pos(arenaLength / 2 - bot_in_corner_frm.x, arenaWidth / 2 - bot_in_corner_frm.y,
...
void ArenaCornerObservation::bindWithRhIO()
{
  ...
  RhIO::Root.newFloat("/localisation/field/ArenaCornerObservation/maxAngleError")
      ->defaultValue(maxAngleError)
      ->minimum(0.0)
      ->maximum(180)
      ->comment("The maximum angle difference between expectation and observation");
  RhIO::Root.newFloat("/localisation/field/ArenaCornerObservation/sigmoidOffset")
      ->defaultValue(sigmoidOffset)
      ->minimum(0.0)
      ->maximum(1.0)
      ->comment("The value at which dScore/dx is lambda, with dx = dAngle/maxAngle");
  RhIO::Root.newFloat("/localisation/field/ArenaCornerObservation/sigmoidLambda")
